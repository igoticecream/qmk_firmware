...
//      Ct_L     Win_L    Alt_L                               SPACE                               Alt_R    FN       Ct_R     Left     Down     Right
    ...
    [1] = LAYOUT(
        RESET,   KC_MYCM, KC_WHOM, KC_CALC, KC_MSEL, KC_MPRV, KC_MNXT, KC_MPLY, KC_MSTP, KC_MUTE, KC_VOLD, KC_VOLU, _______, KC_PAUSE,         KC_MUTE,
        _______, KC_P1,   KC_P2,   KC_P3,   KC_P4,   KC_P5,   KC_P6,   KC_P7,   KC_P8,   KC_P9,   KC_P0,   KC_PMNS, KC_PPLS, _______,          KC_INSERT,
        _______, _______, RGB_VAI, RGB_HUI, _______, _______, _______, _______, _______, _______, _______, KC_PSLS, KC_PAST, _______,          _______,
        _______, _______, RGB_VAD, RGB_HUD, _______, _______, _______, _______, _______, _______, _______, _______,          KC_PENT,          _______,
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    if (index == 0) {  // Check first and only encoder
        switch (get_highest_layer(layer_state)) {
            case 1:  // Layer 1
                if (clockwise) {
                    tap_code(KC_MEDIA_NEXT_TRACK);  // Next track
                } else {
                    tap_code(KC_MEDIA_PREV_TRACK);  // Prev track
                }
                break;
            default:                                       // Layer 0
                if (get_mods() & MOD_MASK_CTRL) {          // If CTRL is held
                    const uint8_t mod_state = get_mods();  // Store all  modifiers that are held
                    ...
                    if (clockwise) {
                        tap_code16(LCTL(KC_RIGHT));  // Desktop switch
                    } else {
                        tap_code16(LCTL(KC_LEFT));
                    }
